...
- (NSData *)encryptDataWithPublicKey:(NSString *)pubKey{
    ...
    if (!pubKey || pubKey.length == 0)
        keyRef = [[SharedKeyPair keypair] getPublicKeyRef];
    else
        keyRef = [self addPublicKey:pubKey];
        
    if(!keyRef){
        return nil;
    }
    return [self encryptData:self withKeyRef:keyRef];
}
...
- (NSData *)decryptDataWithPrivateKey:(NSString *)privKey{
    ...
    if (!privKey || privKey.length == 0)
        keyRef = [[SharedKeyPair keypair] getPrivateKeyRef];
    else
